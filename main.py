...
class Run(object):
    def __init__(self, yaml_dir):
        ...
        self.alpha_weight = self.args.alpha_weight
        self.beta_weight = self.args.beta_weight
    ...
    def train(self, epoch):
        ...
        active_domain_loss_step = self.args.active_domain_loss_step
        dann_epoch = np.floor(active_domain_loss_step / num_iter * 1.0)
            ...
            if (i+1) % int(self.args.print_freq) == 0:
                self.logger.info('Train Epoch[{0}]({1}/{2}): '
                                 ...
                                 'Time: {cost_time:.4f}. '.format(epoch, int(i+1), num_iter, loss_cls=total_cls/(i+1), \
                                                                   loss_diff=total_diff/(i+1),loss_simi=total_simi/(i+1), loss_mse=total_mse/(i+1),
        ...
        self.writer.add_scalar('train_loss_diff', average_diff, epoch)
        self.writer.add_scalar('train_loss_cls', average_cls, epoch)
        self.writer.add_scalar('train_loss_simi', average_simi, epoch)
        self.writer.add_scalar('train_loss_mse', average_mse, epoch)
    ...
    def valid(self, epoch):
        ...
        self.writer.add_scalar('valid_loss_diff', average_diff, epoch)
        self.writer.add_scalar('valid_loss_cls', average_cls, epoch)
        self.writer.add_scalar('valid_loss_simi', average_simi, epoch)
        self.writer.add_scalar('valid_loss_mse', average_mse, epoch)
